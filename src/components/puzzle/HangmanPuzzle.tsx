...
// SVG Hangman drawing - shows parts progressively
function HangmanSVG({ wrongCount, falling }: { wrongCount: number; falling: boolean }) {
  const parts = [
    ...
    <motion.circle
      key="head"
      cx="100" cy="45" r="15"
      stroke="#e2e8f0" strokeWidth="3" fill="none"
  ...
  // Rope segments - cut one by one as wrong answers increase
  ...
  return (
    <svg viewBox="0 0 200 200" className="w-full h-full">
      ...
      {/* Man - animated falling when dead */}
      <motion.g
        animate={falling ? {
          y: [0, 5, 60],
          rotate: [0, -10, 45],
...
export default function HangmanPuzzle({
  ...
  const handleGuess = useCallback((letter: string) => {
    ...
    setGuessed(prev => new Set(Array.from(prev).concat(letter)));  }, [status, guessed]);
  ...
  useEffect(() => {
    if (showHint) {
      ...
      if (unguessed.length > 0) {
        ...
        setGuessed(prev => new Set(Array.from(prev).concat(reveal)));
      }
    }
  }, [showHint]);
        ...
        <div className="flex-1 space-y-4">
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="text-xs text-gray-500 ml-2 font-mono">
              {MAX_WRONG - wrongCount} cuts left
            </span>
          </div>
          ...
          <div className="flex gap-2 flex-wrap">
            {word.split('').map((letter, i) => (
              ...
              >
                <motion.span
                  animate={guessed.has(letter) ? { scale: [1, 1.3, 1] } : {}}
                  className={`text-2xl font-mono font-bold w-8 text-center ${
          ...
          {wrongGuesses.length > 0 && (
            <div className="flex flex-wrap gap-1">
              {wrongGuesses.map(l => (
                <span key={l} className="text-xs font-mono text-red-400 bg-red-500/10 px-1.5 py-0.5 rounded">
                  {l}
                </span>
              ))}
            </div>
